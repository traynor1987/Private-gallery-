...
class BrowserVpnController(
    private val engines: Map<VpnProtocol, VpnEngine>,
    ...
    fun enterBrowser(autoConnect: Boolean, requireVpn: Boolean, now: Long): VpnConnectionState {
        ...
        state = engines.getValue(profile.protocol).connect(profile)
        return state
    }
    ...
    fun leaveBrowser(now: Long) { if (engines.values.any { it.ownsTunnel }) disconnectAt = now + disconnectGraceMillis }
    fun tick(now: Long): VpnConnectionState? {
        if (disconnectAt != null && now >= disconnectAt!!) { disconnectAt = null; activeProfile?.let { state = engines.getValue(it.protocol).disconnect(); return state } }
        return null
    }
    fun tunnelLost(): VpnConnectionState {
        val profile = activeProfile ?: return VpnConnectionState.UNCONFIGURED.also { state = it }
        state = if (reconnectAttempts++ < maxReconnectAttempts) engines.getValue(profile.protocol).connect(profile).let { VpnConnectionState.RECONNECTING } else VpnConnectionState.FAILED
        return state
    }
    fun onLock() { disconnectAt = null; activeProfile?.let { state = engines.getValue(it.protocol).disconnect() } }
}
