...
Provider/model manager, shared editor/settings, isolated CPU runtime and encrypted Save Copy integration passed Android CI run **36144394306** at code commit **5c5d7dc57e39edadc05a2ca9b7410d1a38917c0a**. All **108 Android instrumentation tests** passed, including isolated-UID model descriptor access with Internet permission denied, local prompt tokenizer bounds, local encrypted/restricted Save Copy, local editor provenance, background/lock cancellation and input wiping, and provider-choice migration. Unit tests, lint, arm64/x86_64 native builds, model-distribution checks, inherited-FD host tests, Browser helper tests, secret scan and WireGuard production-path gates passed. CI made no paid requests and downloaded no production weights.

The first CI iterations exposed an API-level manifest mismatch, invalid hyphens in isolated-service names, and Auto configuration being confused with provider-object existence. These were fixed without disabling gates or changing the existing editor assertion. The service is disabled through versioned resources below API 29. The complete suite was rerun successfully.

Neither model has undergone production-weight generation or physical-device acceptance. This is integration/build/security evidence, not a phone performance or image-quality claim. The final main run and permanent-signer artifact run are reported with delivery. Follow `docs/audits/local-ai-physical-acceptance.md` before accepting either provider.

## APK baseline

Starting-main debug APK (`49bb9bc5`, CI run 36134558301): **37,209,501 bytes**. Latest pre-expansion signed acceptance APK (`78be3f35`, run 36117847403): **29,800,290 bytes**. These are extracted APK file sizes, not compressed artifact ZIP sizes. The signed baseline precedes the final video-loading fix; disclose that when comparing. Post-integration debug APK: **138,391,408 bytes** (run 36144394306), an increase of **101,181,907 bytes** over starting main. This universal debug APK includes both reviewed arm64-v8a and x86_64 native runtimes; no weights are present. The signed acceptance workflow independently prints its exact APK byte size, checks both runtimes/no weights, and verifies the permanent signature. Its measured size is reported with delivery; do not extrapolate it from Debug. ABI splits/app bundles could avoid shipping the unused architecture later, but universal signed acceptance packaging is preserved for this milestone.
...
## Final incorporated-source pass

The full pinned native source graph also cites ComfyUI's modified ControlNet implementation (current Comfy-Org/ComfyUI is GPL-3.0) and inclusionAI/LLaDA-Image (no source licence grant found; LEGAL.md is a language disclaimer). Neither is needed by SD1.5/SDXL base. The adaptation removes all ControlNet implementation code, leaving only an independently authored disabled interface, and removes LLaDA model/text/conditioner definitions and factory branch. No ControlNet/LLaDA claim is made. This is exclusion, not an assertion that a reference citation alone proves derivation.

Packaged incorporated-source notices additionally cover NumCpp (MIT; cited commit 5e40aab), llama.cpp (MIT), k-diffusion (MIT), sd3.5 code (MIT; LICENSE-CODE at 8565799), TAESD (MIT), Toyota gradient-estimation-sampler (MIT), Diff-Sampler/GITS, FLUX/FLUX2 code, Ideogram4 code, PixArt Sigma, Lumina, Diffusers, Transformers, Tokenizers and BasicSR (Apache-2.0), Real-ESRGAN (BSD-3-Clause), CPython pickle-format reference (PSF licence), and CompVis source's Open RAIL-M terms. These are incorporated runtime references, not separately enabled app providers or redistributed model weights. Licence source texts were fetched from their actual source repositories; code and weight licences remain separate. Current upstream main licence files are snapshots dated 2026-09-25 where the native citation supplies no source commit; the shipped native implementation itself remains pinned. No extra experimental runtime dependency was added.
