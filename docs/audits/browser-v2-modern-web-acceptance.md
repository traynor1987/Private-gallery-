...
4. **If either still fails:** in that same signed build, repeat only that application with verbose diagnostics OFF after reload, then ON for evidence. If unchanged and native/document focus is false in pre-console interaction snapshots, compare the existing explicit-focus acceptance control once. Keep cookies/TLS/mixed content/VPN policies unchanged. Record which single variable changed, not browsing data.
