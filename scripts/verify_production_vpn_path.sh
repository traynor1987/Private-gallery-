...
if rg -n -i 'ics-openvpn|de\\.blinkt|openvpn3|openvpn 3|agpl|private-gallery-openvpn2' "${production_paths[@]}"; then
  echo 'FAIL: OpenVPN or AGPL material entered the Private Gallery production path' >&2
  exit 1
fi
rg -q 'com.wireguard.android:tunnel' gradle/libs.versions.toml
test -f NOTICE
